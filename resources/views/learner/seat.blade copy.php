        ...
        <div class="col-lg-12">
            <div class="seat-booking">
                @foreach($seats as $seat)
                    <div class="seat">
                        @php
                            $usersForSeat =Learner::leftJoin('learner_detail','learner_detail.learner_id','=','learners.id')->leftJoin('plan_types','learner_detail.plan_type_id','=','plan_types.id')->where('learner_detail.library_id',auth()->user()->id)->where('learners.seat_no', $seat->id)->select('learners.id','learners.seat_no','learner_detail.plan_type_id','plan_types.day_type_id','plan_types.image')->where('learners.status',1)->where('learner_detail.status',1)->get();
                        ...
                        @if($usersForSeat->count() > 0)
                            @foreach($usersForSeat as $user)
                                @if($user->day_type_id == 1)
                                    ...
                                    <ul>
                                        ...
                                        <li><a href="javascript:;" data-bs-toggle="modal"
                                                data-bs-target="#seatAllotmentModal2" data-id="{{ $user->id }}"><i
                                                    class="fa-solid fa-check-circle booked"></i></a></li>
                                    </ul>
                                    <small class="text-dark">Fullday</small>
                                    <img src="{{ asset($user->image) }}" class="bookedSeat" alt="book">
                                    <small class="text-dark">{{ $user->seat_no }}</small>
                                ...
                                @elseif($user->day_type_id == 2)
                                    ...
                                    <small class="text-dark">First Half</small>
                                    <img src="{{ asset($user->image) }}" class="bookedSeat" alt="book">
                                    <small class="text-dark">{{ $halfday++ }}</small>
                                ...
                                @elseif($user->day_type_id == 3)
                                    ...
                                    <small class="text-dark">Second Half</small>
                                    <img src="{{ asset($user->image) }}" class="bookedSeat" alt="book">
                                    <small class="text-dark">{{ $halfday++ }}</small>
                                ...
                                @elseif(in_array($user->day_type_id, [4, 5, 6, 7]))
                                    ...
                                    <small class="text-dark">Hourly</small>
                                    <img src="{{ asset($user->image) }}" class="bookedSeat" alt="book">
                                    <small class="text-dark">{{ $halfday++ }}</small>
                            ...
                            @for ($i = 0; $i < $seatCount; $i++)
                                <ul>
                                    ...
                                    <li><a href="javascript:;" data-bs-toggle="modal"
                                            data-bs-target="#seatAllotmentModal" data-id="{{ $seat->id }}"><i
                                                class="fa-solid fa-check-circle available"></i></a></li>
                                </ul>
                                <small class="text-dark">Available</small>
                                <img src="{{ asset('public/img/available.svg') }}" class="bookedSeat" alt="book">
                                <small class="text-dark">{{ $halfday++ }}</small>
                        ...
                        @else
                            ...
                            <small class="text-dark">Available </small>
                            <img src="{{ asset('public/img/booked.png') }}" class="" alt="book">
